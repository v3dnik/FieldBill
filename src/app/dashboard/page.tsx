    ...
    <div className="max-w-4xl mx-auto px-4 md:px-6 py-6 md:py-8">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
        <div className="bg-white dark:bg-slate-800 border border-gray-200 dark:border-slate-700 rounded-lg p-6">
          <h3 className="text-sm text-gray-500 dark:text-slate-400 mb-2">Ihre Firma</h3>
          <p className="text-lg font-medium text-gray-900 dark:text-white">{company?.name || '—'}</p>
          <p className="text-sm text-gray-500 dark:text-slate-400 mt-1">{company?.phone || '—'}</p>
        </div>
        <div className="bg-white dark:bg-slate-800 border border-gray-200 dark:border-slate-700 rounded-lg p-6">
          <h3 className="text-sm text-gray-500 dark:text-slate-400 mb-2">Ihre Rolle</h3>
          <p className="text-lg font-medium text-gray-900 dark:text-white">
            {isBoss ? 'Geschäftsführer (Chef)' : 'Mitarbeiter'}
          </p>
          <p className="text-sm text-gray-500 dark:text-slate-400 mt-1">{userDoc?.email}</p>
        </div>
      </div>
    </div>
  );
