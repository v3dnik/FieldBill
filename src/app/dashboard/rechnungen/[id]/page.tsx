    ...
    try {
      const res = await fetch('/api/send-invoice-email', {
        ...
        body: JSON.stringify({
          ...
          totalRappen: invoice.totalRappen, companyName: company?.name || '',
      ...
      <div className="relative bg-white dark:bg-gray-900 rounded-2xl shadow-2xl w-full max-w-lg border border-gray-200 dark:border-gray-700">
        ...
        <div className="px-6 py-5 space-y-4">
          <div>
            ...
            <input type="email" value={to} onChange={e => setTo(e.target.value)} placeholder="kunde@example.com"
          ...
          <div className="flex items-start gap-2.5 bg-blue-50 dark:bg-blue-900/20 border border-blue-100 dark:border-blue-500/20 rounded-lg px-3.5 py-3">
            ...
            <p className="text-xs text-blue-700 dark:text-blue-300">Der Kunde erhaelt alle Rechnungsdetails direkt im E-Mail formatiert dargestellt.</p>
          </div>
          {error && (
        ...
        <div className="flex items-center justify-end gap-3 px-6 py-4 border-t border-gray-200 dark:border-gray-700">
          <button onClick={onClose} className="px-4 py-2 text-sm font-medium text-gray-600 dark:text-gray-300 hover:text-gray-900 dark:hover:text-white hover:bg-gray-100 dark:hover:bg-gray-800 rounded-lg transition-colors">Abbrechen</button>
          <button onClick={handleSend} disabled={sending || !to.trim()}
            ...
            {sending ? (
              <><svg className="w-4 h-4 animate-spin" fill="none" viewBox="0 0 24 24"><circle className="opacity-25" cx="12" cy="12" r="10" stroke="currentColor" strokeWidth="4" /><path className="opacity-75" fill="currentColor" d="M4 12a8 8 0 018-8V0C5.373 0 0 5.373 0 12h4z" /></svg>Wird gesendet...</>
            ) : (
              <><svg className="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}><path strokeLinecap="round" strokeLinejoin="round" d="M12 19l9 2-9-18-9 18 9-2zm0 0v-8" /></svg>Senden</>
            )}
          </button>
        </div>
      </div>
...
export default function RechnungDetailPage() {
  ...
  if (error && !invoice) return <div className="max-w-3xl mx-auto px-4 py-8"><div className="bg-red-50 dark:bg-red-500/10 border border-red-200 dark:border-red-500/30 text-red-600 dark:text-red-400 px-4 py-3 rounded-lg">{error}</div></div>;
  if (!invoice) return null;

  return (
    <>
      {showEmailModal && invoice && (
        <EmailModal invoice={invoice} company={company}
          onClose={() => setShowEmailModal(false)}
          onSent={() => { setShowEmailModal(false); setSuccessToast(`E-Mail erfolgreich gesendet an ${invoice.customerEmail || 'Kunde'}.`); }}
        />
      )}
